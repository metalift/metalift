...
def processLoops(header, body, exits, latches, fnArgs):
  ...
  global invNum
  inv = MLInst_Call("inv%s" % invNum, Bool(), *[MLInst_Load(h) for h in havocs], *fnArgs)
  invNum = invNum + 1
  ...
  for b in blocksMap.values():
    ...
    if opcode == "br" and len(ops) > 1:  # a conditional branch
      ...
      for t,v in zip(targets, vals):
        blocksMap[t.name].instructions.insert(0, MLInst_Assume(MLInst(ops[0], v)))

      # default fallthrough
      blocksMap[ops[1].name].instructions.insert(0, MLInst_Assume(
