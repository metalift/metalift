...
def grammar(ci: CodeInfo):
    ...
    if name.startswith("inv"):
        ...
        lst_length = Call("list_length", Int(), read_var)
        lst_sum = Call(LIST_ABS_SUM_FN_NAME, Int(), read_var)
        lst_tail_sum = Call(
            LIST_ABS_SUM_FN_NAME,
            Int(),
            Call("list_tail", Int(), read_var, Add(modified_var, int_lit)),
        )
...
def code_gen(summary: FnDeclRecursive):
    return (
        f'def {summary.name()}({", ".join([str(eval(a)) for a in summary.arguments()])}):\n  '
...
def process_candidate_func(fn_def) -> FnDeclRecursive:
    ...
    args = [Var(a.name, a.type) if isinstance(a, ValueRef) else a for a in args]
    return FnDeclRecursive(
        fn_def.name(), fn_def.returnT(), cast(Eq, fn_def.body()).e2(), *args
    )
...
if __name__ == "__main__":
    ...
    (vars, inv_and_ps, preds, vc, loop_and_ps_info) = analyze(
        filename, fn_name, loops_filename
    )
