...
FnDeclRecursive.codegen = (
    lambda self: f'def {self.name()}({", ".join([str(a.codegen()) for a in self.arguments()])}):\n  '
...
def targetLang():
    ...
    mat_mul = Target(
        MAT_MUL,
        [TupleT(Int(), Int()), TupleT(Int(), Int()), TupleT(Int(), Int())],
        TupleT(Int(), Int()),
...
def runner():
    ...
    r = t.transpile("tests/llvm/no_loop_matmul.ll", "test")
