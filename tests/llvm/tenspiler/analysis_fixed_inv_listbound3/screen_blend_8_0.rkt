...
(define-grammar (screen_blend_8_inv0_gram active agg.result base col pixel row row_vec)
 [rv (choose (&& (&& (>= row 0 ) (<= row (length base ) ) ) (equal? agg.result (nested_list_elemwise_sub (nested_list_elemwise_add (list-take-noerr base row ) (list-take-noerr active row )) (nested_list_scalar_div 255 (nested_list_elemwise_mul (list-take-noerr base row ) (list-take-noerr active row )))) ) ))]

)

(define-grammar (screen_blend_8_inv1_gram active base col pixel row_vec agg.result row)
 [rv (choose (&& (&& (&& (&& (&& (>= row 0 ) (< row (length base ) ) ) (>= col 0 ) ) (<= col (length (list-list-ref-noerr base 0 ) ) ) ) (equal? row_vec (vec_elemwise_sub (vec_elemwise_add (list-take-noerr (list-list-ref-noerr base row ) col ) (list-take-noerr (list-list-ref-noerr active row ) col )) (vec_scalar_div 255 (vec_elemwise_mul (list-take-noerr (list-list-ref-noerr base row ) col ) (list-take-noerr (list-list-ref-noerr active row ) col )))) ) ) (equal? agg.result (nested_list_elemwise_sub (nested_list_elemwise_add (list-take-noerr base row ) (list-take-noerr active row )) (nested_list_scalar_div 255 (nested_list_elemwise_mul (list-take-noerr base row ) (list-take-noerr active row )))) ) ))]
