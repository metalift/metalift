...
from tests.llvm.hardlift.hardlift_common import (call_integer_exp,
                                                 call_scalar_vec_div,
                                                 call_scalar_vec_sub,
...
def transformer_part3_ps_grammar(writes: List[Object], reads: List[Object], in_scope: List[Object]) -> Bool:
    ret_val = writes[0]
    ...
    return ret_val == call_vec_elemwise_mul(
        ...
        call_scalar_vec_div(
            cons,
            call_vec_scalar_add(cons, call_vec_map(negative_vec, map_int_to_int_fn_obj))
        ),
    )

def transformer_part3_inv0_grammar(writes: List[Object], reads: List[Object], in_scope: List[Object]) -> Bool:
    input, hidden_dim = reads
    out, _, i = writes
    vec = choose(input[:i])
    negative_vec = call_scalar_vec_sub(Int(0), vec)
...
if __name__ == "__main__":
    ...
    transformer_part3 = driver.analyze(
        ...
        loops_filepath="tests/llvm/hardlift/llama/cpp/transformer/transformer_part3.loops",
        fn_name="transformer_part3",
        target_lang_fn=transformer_part3_target_lang,
        inv_grammars={"transformer_part3_inv0": InvGrammar(transformer_part3_inv0_grammar, [])},
