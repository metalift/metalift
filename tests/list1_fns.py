...
def grammar(ci: CodeInfo):
  ...
  if name.startswith("inv"):
    ...
    e = Choose(Call("list_eq",Bool(),Call('list_concat',List(Int()),a,Call("Select", List(Int()), Call('list_tail', List(Int()),a,ci.modifiedVars[1]),f)),a), Call("list_eq",Bool(),Call('list_concat',List(Int()),a, Call('list_tail', List(Int()),a,ci.modifiedVars[1])),a))
...
def targetLang():
  ...
  data = Var("l", List(Int()))
  f = Var("f", Fn(Bool()))
  select_func = FnDecl("Select", List(Int()), Ite(Eq(Call("list_length",Int(),data),IntLit(0)), Call("list_empty",List(Int())), Ite(Call(f.args[0],Fn(Bool()),Call("list_get", Int(), data, IntLit(0))),Call("list_append",List(Int()),Call("Select",List(Int()),Call("list_tail", List(Int()),data, IntLit(1)),f), Call("list_get", Int(),data, IntLit(0)),),Call("Select",List(Int()),Call("list_tail",List(Int()),data,IntLit(1)),f))),data,f)
