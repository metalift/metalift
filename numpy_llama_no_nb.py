...
# @njit
def multiquery_attention_part2_einsum_np(
    token_position, head, head_size, key_cache_layer, attention
):
    return np.einsum(
        "ij,i->j",
...
# @njit
def rmsnorm_part1_dot(input, weight):
...
def rmsnorm_part1_dot_runner(weights, inputs, f=None):
    total_time = 0
    for i in range(len(inputs)):
        inp = inputs[i].flatten()
        w = weights[i].flatten()
...
random_k_weights = [np.random.rand(8192, 8192).astype(np.float32) for _ in range(100)]
random_q_weights = [np.random.rand(8192, 8192).astype(np.float32) for _ in range(100)]
timer(random_k_weights, random_q_weights, None, multiquery_attention_part2_runner)
random_k_weights = [np.random.rand(8192, 8192).astype(np.float32) for _ in range(100)]
random_q_weights = [np.random.rand(8192, 8192).astype(np.float32) for _ in range(100)]
timer(
    random_k_weights, random_q_weights, None, multiquery_attention_part2_einsum_runner
)
