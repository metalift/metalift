...
def transformer_part1_torch (token_position, head, head_size, key_cache_layer, q):
    return (torch.matmul(key_cache_layer[0:token_position][:, (head) * (head_size):(head) * (head_size) + head_size], q[(head) * (head_size):(head) * (head_size) + head_size])) / (torch.sqrt(torch.as_tensor((head_size) * (1))))
...
for _ in range(runs):
    ...
    for i in range(len(q_weights)):
        ...
        q = q.to(device)
        res = transformer_part1_torch(token_position, head, head_size, key_cache_layer, q)
        res = res.to(cpu)
