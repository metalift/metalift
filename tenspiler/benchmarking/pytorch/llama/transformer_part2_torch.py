...
def transformer_part2_torch (token_position, head, head_size, key_cache_layer, attention):
    return torch.matmul(torch.transpose(key_cache_layer[0:(token_position) + (1)][:, (head) * (head_size):(head) * (head_size) + head_size], 0, 1), attention[:(token_position) + (1)])
...
for _ in range(runs):
    ...
    for i in range(len(q_weights)):
        ...
        attention = transformer_part1_torch(token_position, head, head_size, key_cache_layer, q).to(cpu)
        attention = torch.cat((attention, torch.tensor([0]).to(cpu))).to(cpu)
        
        start_time = time.perf_counter()
        key_cache_layer = key_cache_layer.to(device)
        attention = attention.to(device)
        res = transformer_part2_torch(token_position, head, head_size, key_cache_layer, attention)
        res = res.to(cpu)
