...
gemmini_not_comp = ['translate_array', 'vcopy', 'vmul', 'vneg', 'voffset', 'vrecip', 'diveq', 'muleq', 'negate', 'subeq_sca', 'array_inc', 'cube_in_place', 'fourth_in_place']
gemmini_blend_func_names = ['normal_blend_8', 'normal_blend_f', 'linear_burn_8', 
                    'linear_dodge_8', 'screen_blend_8']
...
def setup_gemmini(src_filepath, filename):
    ...
    try:
        result = subprocess.run([gemmini_build_path], capture_output=True, text=True, shell=True)
...
def run_gemmini(filename):
    ...
    try:
        result = subprocess.run([gemmini_run_script_path, filename], capture_output=True, text=True, shell=True)
...
def run_file(filename):
    ...
    if not cpp_exec_files:
        print(f"Expected {filename}_O3_GEM not found in {cpp_dir}. Trying to compile the C++ file.")
    ...
    gemmini_kernel_runtime = gemmini_runtime_conversion.get(filename, 10000) * gemmini_kernel_cycle / (10**9) * 1000
    ...
    gemmini_e2e_runtime = gemmini_runtime_conversion.get(filename, 10000) * gemmini_e2e_cycle / (10**9) * 1000
...
def main():
    ...
    if filename == 'ALL':
        ...
        print(f"Average kernel speedup of all in Gemmini is: {np.mean(all_kernel_speedup)}")
