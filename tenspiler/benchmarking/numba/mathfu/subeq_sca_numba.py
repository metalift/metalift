...
@cuda.jit()
def subeq_sca_numba(a, b, n):
    for i in range(n):
        temp = a[i] - b


def subeq_sca_numba_glued(a, b, n):
    a = np.array(a).astype(np.int32)
    return subeq_sca_numba(a, b, n)
...
subeq_sca_numba[blockspergrid, threadsperblock](b, v, n)
