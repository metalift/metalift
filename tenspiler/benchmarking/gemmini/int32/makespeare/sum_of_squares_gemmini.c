...
void sum_of_squares_gemmini(elem_t arr[LEN][LEN], elem_t n, elem_t* out){
...
int32_t sum_of_squares_gemmini_glued (int32_t arr[LEN], int32_t n){
    ...
    elem_t out;
    sum_of_squares_gemmini(glued_6, n, &out);
...
int main() {
    ...
    elem_t out;
    start = read_cycles();
    sum_of_squares_gemmini(w, n, &out);
    end = read_cycles();
    totalTime += end - start;
    
  
  
    printf("sum_of_squares_gemmini_gemmini");
    printf("%llu\n", totalTime);
    printf("%llu\n", totalTime);
    exit(0);
}
