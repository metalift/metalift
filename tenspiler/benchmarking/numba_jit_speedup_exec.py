...
python_dir = os.path.join(parent_path, "numba_jit")
...
def run_file(filename):
    ...
    python_files = find_file(python_dir, filename + "_numba_jit.py")
...
def get_numba_jit_speedup(benchmark_name: str) -> None:
    if benchmark_name == "ALL":
        ...
        # print(
        #     f"Average kernel speedup of all in Numba Jit is: {np.mean(all_kernel_speedup)}"
        # )
        # print(f"Average E2E speedup of all in Numba Jit is: {np.mean(all_e2e_speedup)}")
    else:
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("Usage: python numba_jit_speedup_exec.py <benchmark name>")
        exit(1)

    get_numba_jit_speedup(sys.argv[1])
