...
@jit(nopython=True)
def lmsfir2_numba_jit(NTAPS, input, coefficient, error):
    output = np.empty(NTAPS - 1)
    for i in range(NTAPS - 1):
        output[i] = coefficient[i] + ((error) * (input[i]))
    return output
