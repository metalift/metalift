...
@jit(nopython=True)
def transformer_part2_numba_jit (token_position, head, head_size, key_cache_layer, attention):
    # xb = []
    xb = np.empty(head_size)
...
def transformer_part1_numba_jit (token_position, head, head_size, key_cache_layer, q):
...
attention = transformer_part1_numba_jit(token_position, head, head_size, key_cache_layer, q)
attention = np.append(attention, np.array([0]))



transformer_part2_numba_jit(token_position, head, head_size, key_cache_layer, attention)
...
print("transformer_part2_numba_jit")
