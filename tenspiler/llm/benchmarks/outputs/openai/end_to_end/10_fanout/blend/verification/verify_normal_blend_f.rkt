...
(list-tail-noerr (list-take-noerr lst end ) start ))
 ...
 (define-bounded (normal_blend_f_ps active base opacity out)
(equal? out (vec_elemwise_add (vec_scalar_mul opacity active) (vec_scalar_mul (- 1 opacity ) base)) ))

out 0
active, base (1 1)
 i+ 1= 1
 (1) :5 = 1
(define-bounded (normal_blend_f_inv0 active base i opacity out)
(&& (>= i 0 ) (&& (<= i (length base ) ) (equal? out (vec_elemwise_add (vec_scalar_mul opacity (list-take-noerr active i )) (vec_scalar_mul (- 1 opacity ) (list-take-noerr base i ))) ) ) ))
...
(define normal_blend_f_rv (take (list normal_blend_f_rv_BOUNDEDSET-0 normal_blend_f_rv_BOUNDEDSET-1) normal_blend_f_rv_BOUNDEDSET-len))
(define-symbolic opacity integer?)
(define-symbolic ref.tmp integer?)
(current-bitwidth 6)
(define vc (verify (assert (&& (&& (=> (&& (equal? (length base ) (length active ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base 0 opacity (list-empty )) ) (=> (&& (&& (&& (< i (length base ) ) (equal? (length base ) (length active ) ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base i opacity agg.result) ) (normal_blend_f_inv0 active base (+ i 1 ) opacity (list-append agg.result (+ (* opacity (list-ref-noerr active i ) ) (* (- 1 opacity ) (list-ref-noerr base i ) ) ) )) ) ) (=> (or (&& (&& (&& (! (< i (length base ) ) ) (equal? (length base ) (length active ) ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base i opacity agg.result) ) (&& (&& (&& (&& (! true ) (! (< i (length base ) ) ) ) (equal? (length base ) (length active ) ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base i opacity agg.result) ) ) (normal_blend_f_ps active base opacity agg.result) ) ))))

; (define vc (verify (=> (&& (equal? (length base ) (length active ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base 0 opacity (list-empty )) )))

(define vc (verify (assert (=>
    (&& (&& (&& (< i (length base ) ) (equal? (length base ) (length active ) ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base i opacity agg.result) )
    (normal_blend_f_inv0 active base (+ i 1 ) opacity (list-append agg.result (+ (* opacity (list-ref-noerr active i ) ) (* (- 1 opacity ) (list-ref-noerr base i ) ) ) ))
))))

vc
; (evaluate (list-take-noerr (list-append agg.result (+ (* opacity (list-ref-noerr active i ) ) (* (- 1 opacity ) (list-ref-noerr base i ) ) ) ) 1) vc)
; (evaluate (vec_elemwise_add (vec_scalar_mul opacity (list-take-noerr active 1 )) (vec_scalar_mul (- 1 opacity ) (list-take-noerr base 1 ))) vc)
; (define vc (verify (assert (=> (or (&& (&& (&& (! (< i (length base ) ) ) (equal? (length base ) (length active ) ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base i opacity agg.result) ) (&& (&& (&& (&& (! true ) (! (< i (length base ) ) ) ) (equal? (length base ) (length active ) ) ) (> (length base ) 1 ) ) (normal_blend_f_inv0 active base i opacity agg.result) ) ) (normal_blend_f_ps active base opacity agg.result) ))))
; vc
; (evaluate base vc)
; (evaluate active vc)
; (evaluate opacity vc)
; (evaluate agg.result vc)
; (evaluate i vc)
