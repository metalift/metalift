...
def list_list_slice(matrix: List[List[int]], start: int, end: int) -> List[List[int]]:
    return matrix[:end][:start]


def list_list_slice_with_length(
    matrix: List[List[int]], start: int, lst_length: int
) -> List[List[int]]:
    return matrix[start : start + lst_length]


def list_list_col_slice(
    matrix: List[List[int]], start: int, end: int
) -> List[List[int]]:
    return (
        []
        if len(matrix) < 1
        else [matrix[0][start:end], *list_list_col_slice(matrix[1:], start, end)]
    )


def list_list_col_slice_with_length(
    matrix: List[List[int]], start: int, lst_length: int
) -> List[List[int]]:
    return list_list_col_slice(matrix, start, start + lst_length)
...
def rests(matrix: List[List[int]]) -> List[List[int]]:
    return [] if len(matrix) < 1 else list_list_col_slice(matrix, 1, len(matrix[0]))
