...
(define-grammar (dissolve_blend_8_inv1_gram active base col opacity pixel rand_cons rand_val row_vec agg.result row)
 [rv (choose (&& (&& (&& (&& (&& (>= row 0 ) (<= row (matrix-length base ) ) ) (>= col 0 ) ) (<= col (length (matrix-ref-noerr base 0 ) ) ) ) (equal? row_vec (dissolve_selection_two_args (if (OUTER_LOOP_INDEX_FIRST) (list-take-noerr (matrix-ref-noerr (v0) row ) col ) (matrix-ref-noerr (matrix-transpose-noerr (matrix-col-slice-with-length-noerr (matrix-take-noerr (v0) col ) row 1 ) ) 0 ) ) (if (OUTER_LOOP_INDEX_FIRST) (list-take-noerr (matrix-ref-noerr (v0) row ) col ) (matrix-ref-noerr (matrix-transpose-noerr (matrix-col-slice-with-length-noerr (matrix-take-noerr (v0) col ) row 1 ) ) 0 ) ) opacity rand_cons dissolve_select_two_args) ) ) (equal? agg.result (dissolve_matrix_selection_two_args (v1) (v1) opacity rand_cons dissolve_select_two_args) ) ))]
...
(define-symbolic dissolve_blend_8_rv_BOUNDEDSET-4 integer?)
(define dissolve_blend_8_rv (take (list dissolve_blend_8_rv_BOUNDEDSET-0 dissolve_blend_8_rv_BOUNDEDSET-1 dissolve_blend_8_rv_BOUNDEDSET-2 dissolve_blend_8_rv_BOUNDEDSET-3 dissolve_blend_8_rv_BOUNDEDSET-4) dissolve_blend_8_rv_BOUNDEDSET-len))
