...
def inv0_grammar_fn(
    ...
    return and_objects(
        ...
        == call_elemwise_add(
            ...
            call_elemwise_mul(
                call_elemwise_add(
                    tensor3d[:i], call_elemwise_mul(tensor3d[:i], tensor3d[:i])
                ),
                tensor3d[:i],
            ),
        ),
...
def inv1_grammar_fn(
    ...
    return and_objects(
        ...
        == call_elemwise_add(
            ...
            call_elemwise_mul(
                call_elemwise_add(
                    tensor3d[i][:j], call_elemwise_mul(tensor3d[i][:j], tensor3d[i][:j])
                ),
                tensor3d[i][:j],
            ),
        ),
    ...
    return and_objects(
        ...
        == call_elemwise_add(
            ...
            call_elemwise_mul(
                call_elemwise_add(
                    tensor3d[i][j][:k],
                    call_elemwise_mul(tensor3d[i][j][:k], tensor3d[i][j][:k]),
                ),
                tensor3d[i][j][:k],
            ),
        ),
...
def ps_grammar_fn(
    ...
    return result == call_elemwise_add(
        tensor3d,
        call_elemwise_mul(
            call_elemwise_add(tensor3d, call_elemwise_mul(tensor3d, tensor3d)), tensor3d
        ),
    )
...
if __name__ == "__main__":
    ...
    elemwise_depth5 = driver.analyze(
        llvm_filepath="tenspiler/3d_artificial/cpp/elemwise_depth5.ll",
        loops_filepath="tenspiler/3d_artificial/cpp/elemwise_depth5.loops",
        fn_name="elemwise_depth5",
        target_lang_fn=target_lang,
        inv_grammars={
            "elemwise_depth5_inv0": inv0_grammar,
            "elemwise_depth5_inv1": inv1_grammar,
            "elemwise_depth5_inv2": inv2_grammar,
        },
    ...
    elemwise_depth5(tensor3d_x, tensor3d_y)
    driver.synthesize(
        filename="elemwise_depth5",
        list_bound=2,
        relaxed=False,
        rounds_to_guess=0,
        no_verify=True,
