...
def transformer_part1_mx (token_position, head1, head_size, key_cache_layer, q):
    return (mx.matmul(mx.transpose(key_cache_layer[0:token_position][:, (head) * (token_position):((head) * (token_position)) + (head_size)]), q[(head) * (token_position):((head) * (token_position)) + (head_size)])) / ((0) * (head_size))
