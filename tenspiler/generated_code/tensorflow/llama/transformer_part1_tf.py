...
def transformer_part1_tf(token_position, head1, head_size, key_cache_layer, q):
    return (tf.linalg.matvec(key_cache_layer[0:token_position][:, (head1) * (head_size):((head1) * (head_size)) + (head_size)], q[(head1) * (head_size):((head1) * (head_size)) + (head_size)])) / (tf.sqrt(tf.cast((1) * (head_size), tf.float32)))
