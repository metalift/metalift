...
def negate_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    driver = Driver()
    negate = driver.analyze(
        "tenspiler/c2taco/cpp/for_synthesis/mathfu/negate.ll",
        "tenspiler/c2taco/cpp/for_synthesis/mathfu/negate.loops",
        "negate",
        negate_target_lang,
        defaultdict(lambda: InvGrammar(negate_inv0_grammar, [])),
        negate_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"negate{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
