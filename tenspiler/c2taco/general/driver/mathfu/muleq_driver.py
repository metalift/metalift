...
def muleq_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    driver = Driver()
    muleq = driver.analyze(
        "tenspiler/c2taco/cpp/for_synthesis/mathfu/muleq.ll",
        "tenspiler/c2taco/cpp/for_synthesis/mathfu/muleq.loops",
        "muleq",
        muleq_target_lang,
        defaultdict(lambda: InvGrammar(muleq_inv0_grammar, [])),
        muleq_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"muleq{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
