...
def muleq_sca_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
def muleq_sca_ps_grammar(
    writes: List[Object], reads: List[Object], in_scope: List[Object]
) -> Bool:
    a, b, n = reads
    out = writes[0]
    int_var = choose(Int(0), n, b).maybe_relaxed(parser_args.relaxed)
...
def muleq_sca_inv0_grammar(
    ...
    lower_bound = Int(0)
    upper_bound = n
    int_var = choose(lower_bound, upper_bound, i, b).maybe_relaxed(parser_args.relaxed)
...
if __name__ == "__main__":
    ...
    driver = Driver()
    muleq_sca = driver.analyze(
        "tenspiler/c2taco/cpp/for_synthesis/mathfu/muleq_sca.ll",
        "tenspiler/c2taco/cpp/for_synthesis/mathfu/muleq_sca.loops",
        "muleq_sca",
        muleq_sca_target_lang,
        defaultdict(lambda: InvGrammar(muleq_sca_inv0_grammar, [])),
        muleq_sca_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"muleq_sca{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
