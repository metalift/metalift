...
def subeq_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    subeq = driver.analyze(
        ...
        "subeq",
        subeq_target_lang,
        defaultdict(lambda: InvGrammar(subeq_inv0_grammar, [])),
        subeq_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"subeq{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
