...
def len_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    len = driver.analyze(
        ...
        "len",
        len_target_lang,
        defaultdict(lambda: InvGrammar(len_inv0_grammar, [])),
        len_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"len{depth_suffix}{relaxed_suffix}", rounds_to_guess=9)
    end_time = time.time()
