...
def len_sq_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
def len_sq_ps_grammar(
    ...
    return l == vec_to_int(vec)


def len_sq_inv0_grammar(
    writes: List[Object], reads: List[Object], in_scope: List[Object]
) -> Bool:
    arr, n = reads
    i, l = writes
...
if __name__ == "__main__":
    ...
    len_sq = driver.analyze(
        ...
        "len_sq",
        len_sq_target_lang,
        defaultdict(lambda: InvGrammar(len_sq_inv0_grammar, [])),
        len_sq_ps_grammar,
    )
    ...
    driver.synthesize(
        filename=f"len_sq{depth_suffix}{relaxed_suffix}", rounds_to_guess=9
    )
    end_time = time.time()
