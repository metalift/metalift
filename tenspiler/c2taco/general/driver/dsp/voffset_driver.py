...
def voffset_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    voffset = driver.analyze(
        ...
        "voffset",
        voffset_target_lang,
        defaultdict(lambda: InvGrammar(voffset_inv0_grammar, [])),
        voffset_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"voffset{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
