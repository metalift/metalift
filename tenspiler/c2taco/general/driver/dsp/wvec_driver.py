...
def wvec_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
def wvec_ps_grammar(
    ...
    a, b, m, n = reads
    out = writes[0]
    int_var = choose(Int(0), n, m).maybe_relaxed(parser_args.relaxed)
...
def wvec_inv0_grammar(
    ...
    lower_bound = Int(0)
    upper_bound = n
    int_var = choose(lower_bound, upper_bound, i, m).maybe_relaxed(parser_args.relaxed)
...
if __name__ == "__main__":
    ...
    wvec = driver.analyze(
        ...
        "wvec",
        wvec_target_lang,
        defaultdict(lambda: InvGrammar(wvec_inv0_grammar, [])),
        wvec_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"wvec{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
