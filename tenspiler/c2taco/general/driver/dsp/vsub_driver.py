...
def vsub_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    vsub = driver.analyze(
        ...
        "vsub",
        vsub_target_lang,
        defaultdict(lambda: InvGrammar(vsub_inv0_grammar, [])),
        vsub_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"vsub{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
