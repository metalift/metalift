...
def vmul_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    vmul = driver.analyze(
        ...
        "vmul",
        vmul_target_lang,
        defaultdict(lambda: InvGrammar(vmul_inv0_grammar, [])),
        vmul_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"vmul{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
