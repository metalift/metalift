...
def vcopy_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    vcopy = driver.analyze(
        ...
        "vcopy",
        vcopy_target_lang,
        defaultdict(lambda: InvGrammar(vcopy_inv0_grammar, [])),
        vcopy_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"vcopy{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
