...
def vadd_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    vadd = driver.analyze(
        ...
        "vadd",
        vadd_target_lang,
        defaultdict(lambda: InvGrammar(vadd_inv0_grammar, [])),
        vadd_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"vadd{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
