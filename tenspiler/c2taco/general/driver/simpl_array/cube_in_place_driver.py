...
def cube_in_place_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
def cube_in_place_inv0_grammar(
    writes: List[Object], reads: List[Object], in_scope: List[Object], relaxed: bool
) -> Bool:
    arr, n = reads
    out, i, _ = writes
...
if __name__ == "__main__":
    ...
    cube_in_place = driver.analyze(
        ...
        "cube_in_place",
        cube_in_place_target_lang,
        defaultdict(lambda: InvGrammar(cube_in_place_inv0_grammar, [])),
        cube_in_place_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"cube_in_place{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
