...
def array_inc_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    array_inc = driver.analyze(
        ...
        "array_inc",
        array_inc_target_lang,
        defaultdict(lambda: InvGrammar(array_inc_inv0_grammar, [])),
        array_inc_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"array_inc{depth_suffix}{relaxed_suffix}")
    end_time = time.time()
