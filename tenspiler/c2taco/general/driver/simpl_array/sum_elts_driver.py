...
def sum_elts_target_lang() -> List[Union[FnDecl, FnDeclRecursive]]:
    return [
        *vec_vec_to_vec_target_lang,
        *scalar_vec_to_vec_target_lang,
        *vec_to_vec_target_lang,
...
if __name__ == "__main__":
    ...
    sum_elts = driver.analyze(
        ...
        "sum_elts",
        sum_elts_target_lang,
        defaultdict(lambda: InvGrammar(sum_elts_inv0_grammar, [])),
        sum_elts_ps_grammar,
    )
    ...
    driver.synthesize(filename=f"sum_elts{depth_suffix}{relaxed_suffix}", list_bound=3)
    end_time = time.time()
