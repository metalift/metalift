...
translations = {
    ...
    "matrix_prepend": lambda processed_args: f"np.concatenate([np.expand_dims({processed_args[1]}, axis=0), {processed_args[0]}], axis=0)",
    ...
    "matrix_slice": lambda processed_args: f"{processed_args[0]}[{processed_args[1]}:{processed_args[2]}]",
    "list_slice_with_length": lambda processed_args: f"{processed_args[0]}[{processed_args[1]}:{processed_args[1]} + {processed_args[2]}]",
    "matrix_slice_with_length": lambda processed_args: f"{processed_args[0]}[{processed_args[1]}:{processed_args[1]} + {processed_args[2]}]",
...
def numpy_codegen(
    ...
    for i in range(len(arguments)):
        if argument_types[i] == Matrix[Int] or argument_types[i] == mlList[Int]:
            ...
            conversions.append(f"{arguments[i]} = np.array({arguments[i]}).astype({lib_dtype})")
