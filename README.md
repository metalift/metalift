...
The tutorial uses **Bedrock**. Authenticate by mounting your `~/.aws` folder into the container so boto3 can read credentials from `~/.aws/credentials` and `~/.aws/config`:

```bash
docker run --rm -v ~/.aws:/root/.aws:ro llmlift-tutorial python tenspiler/llama/llm/driver/softmax_part1_driver.py
```
